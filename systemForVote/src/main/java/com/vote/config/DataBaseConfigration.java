...
public class DataBaseConfigration implements EnvironmentAware {
	
	private final Logger log = LoggerFactory.getLogger(DataBaseConfigration.class);
	...
	public DataSource dataSource() {
		log.debug("configing datasource");
		log.info(propertyResolver.getProperty("url"));
		log.info(propertyResolver.getProperty(".url"));
		log.info(propertyResolver.getProperty("spring.datasource.url"));
		String url = propertyResolver.getProperty("url");
		String name = propertyResolver.getProperty("databaseName");
		...
		if(StringUtils.isEmpty(propertyResolver.getProperty(".url"))) {
			...
			config.addDataSourceProperty("severName", propertyResolver.getProperty(".severName"));
		} else {
		...
		if(env.acceptsProfiles(Constants.SPRING_PROFILE_FAST)) {
			if("org.h2.jdbcx.JdbcDataSource".equals(propertyResolver.getProperty(".dataSourceClassName"))) {
                ...
                log.warn("Using '{}' profile with H2 database in memory is not optimal, you should consider switching to" +
                    " MySQL or Postgresql to avoid rebuilding your database upon each start.", "fast");
			} else {
				liquibase.setShouldRun(false);
			}
		} else {
			log.debug("configing liquibase");
		}
		return liquibase;
	}
